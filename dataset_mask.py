...
class LiDAR2DCompletionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Returns:
            partial: shape (n_beams,) - 欠損あり距離データ
...
if __name__ == "__main__":
    ...
    for partial, mask, target in train_loader:
        ...
        print(f"  Observed points (mask=1): {mask[0].sum().item():.0f}")
        print(f"  Occluded points (mask=0): {(mask[0] == 0).sum().item():.0f}")
        print(f"  Occlusion rate: {(mask[0] == 0).sum().item() / len(mask[0]):.2%}")
